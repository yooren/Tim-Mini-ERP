...
const crm = {
  ...
  renderOpportunityList() {
    ...
    <div class="table-wrap"><table class="data-table"><thead><tr><th>编号</th><th>名称</th><th>阶段</th><th>金额</th><th>概率</th><th>负责人</th><th>预计日</th><th>状态</th><th>操作</th></tr></thead><tbody>
      ${paged.length?paged.map(o=>`<tr><td><span style="font-weight:600">${o.code}</span></td><td>${o.name}</td><td><span style="padding:2px 8px;background:${this.getStageColor(o.stage)}22;color:${this.getStageColor(o.stage)};border-radius:4px;font-weight:600;font-size:12px">${o.stage}</span></td><td>¥${(o.amount||0).toLocaleString()}</td><td><span style="color:${o.probability>=70?'#22c55e':o.probability>=40?'#f59e0b':'#ef4444'};font-weight:600">${o.probability}%</span></td><td>${o.owner||'-'}</td><td>${o.expectedDate||'-'}</td><td>${this.getBadge(o.status,'opportunity')}</td><td><button class="btn btn-outline btn-sm" onclick="crm.advanceStage(${o.id})">推进</button>${o.status==='进行中'?`<button class="btn btn-success btn-sm" onclick="crm.winOpportunity(${o.id})">赢单</button><button class="btn btn-danger btn-sm" onclick="crm.loseOpportunity(${o.id})">输单</button>`:''}<button class="btn btn-outline btn-sm" onclick="crm.openEditOpportunity(${o.id})">编辑</button><button class="btn btn-danger btn-sm" onclick="if(!hasPerm('crm','delete')){toast('没有删除权限','error');}else{DB.delete('crmOpportunities',${o.id});toast('已删除','warning');crm.reload();}">删除</button></td></tr>`).join(''):''}
    </tbody></table></div>
    ${paged.length?renderPagination(total,this.page,this.pageSize,'crm.goPage'):''}
  ...
  openEditOpportunity(id) {
    ...
    openModal('编辑商机',`<div class="form-row cols-2"><div class="form-item"><label>名称</label><input id="opeName" value="${o.name}"></div><div class="form-item"><label>负责人</label><input id="opeOwner" value="${o.owner||''}"></div></div><div class="form-row cols-2"><div class="form-item"><label>阶段</label><select id="opeStage" style="width:100%;padding:10px;border:1.5px solid var(--border);border-radius:8px;background:var(--bg)"><option value="初步接触" ${o.stage==='初步接触'?'selected':''}>初步接触</option><option value="需求确认" ${o.stage==='需求确认'?'selected':''}>需求确认</option><option value="方案制定" ${o.stage==='方案制定'?'selected':''}>方案制定</option><option value="商务谈判" ${o.stage==='商务谈判'?'selected':''}>商务谈判</option><option value="合同签订" ${o.stage==='合同签订'?'selected':''}>合同签订</option></select></div><div class="form-item"><label>概率</label><input id="opeProb" type="number" value="${o.probability||10}"></div></div><div class="form-row cols-2"><div class="form-item"><label>金额</label><input id="opeAmount" type="number" value="${o.amount||0}"></div><div class="form-item"><label>预计日</label><input id="opeDate" type="date" value="${o.expectedDate||''}"></div></div>`,
      `<button class="btn btn-ghost" onclick="closeModal()">取消</button><button class="btn btn-primary" onclick="DB.update('crmOpportunities',${id},{name:document.getElementById('opeName').value.trim(),owner:document.getElementById('opeOwner').value.trim(),stage:document.getElementById('opeStage').value,probability:parseInt(document.getElementById('opeProb').value)||10,amount:parseFloat(document.getElementById('opeAmount').value)||0,expectedDate:document.getElementById('opeDate').value});closeModal();toast('已更新！');crm.reload()">保存</button>`);
  },
  ...
  exportCurrentTab() {
    ...
    switch(tab) {
      ...
      case 'opportunity': {
        ...
        rows = list.map(o => [o.code, o.name, o.clientName||'', o.stage||'', o.amount||0, o.probability||'', o.owner||'', o.status||'']);
        filename = 'CRM商机列表';
        break;
      }
      case 'contract': {
