// 数据库访问层 - SQLite 实现：使用 Node.js 内置 node:sqlite（Node 22.5+ 自带，无需原生编译依赖）。
// schema 设计与前端 localStorage 的 "账套(account) + 集合(collection) => JSON数组" 模型
// 一一对应，前端 js/data.js 的 get/set 语义可以零改造地映射到 HTTP API。
//
// 所有方法都写成 async（虽然 node:sqlite 本身是同步 API），是为了跟 db-mysql.js
// 暴露完全一样的接口，server.js 不需要关心当前用的是哪个后端。
